...
const bcrypt = require('bcriptjs') // Подключаем библиотеку по хешированию паролей bcrypt
...
router.post(
  ...
  [ // Массив midlwares для валидации наших данных
    ...
    check('password', 'Minimum password length is 8 symbols').isLength({ // Проверяем длину пароля
      min: 6,
      max: 16,
    }),
  ],
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty) { // Проверка на успешное завершение валидации
        return res.status(400).json({ // Отправляем ошибку на frontend
          errors: errors.array(),
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty) { // Проверка на успешное завершение валидации
        return res.status(400).json({ // В случае ошибки, выкидываем ее в frontend
          errors: errors.array(),
      ...
      if (!user) { // Если такого пользователя нет, то выкмдываем ошибку в frontend
      ...
      if (!isMatch) { // В случае неверного пароля выкидываем ошибку в frontend
